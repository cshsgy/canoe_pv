  ...
  */
  for (int k = ks; k <= ke; ++k)
    for (int j = js; j <= je; ++j){
      ...
      du(IEN, k, j, is) += hrate * dt / dz * (1. + 1.E-4 * sin(2. * M_PI * rand() / RAND_MAX));;
      ...
      for (int i = is; i<=ie; ++i){
        if (w(IPR, k, j, i) < prad){ // relax to Tmin profile
          ...
          // du(IEN, k, j, i) += (Tmin - Tnow) * cv * w(IDN, k, j, i) * dt / 1E5;
        }
      }
      // du(IEN, k, j, ie) -= hrate * dt / dz * (1. + 1.E-4 * sin(2. * M_PI * rand() / RAND_MAX));;
    }
    ...
    for (int j = pmb->js; j <= pmb->je; ++j) {
      for (int i = pmb->is; i <= pmb->ie; ++i) {
        ...
        if (s < 1) { 
          ...
          du(IM1,k,j,i) -= dt*w(IDN,k,j,i)*w(IVX,k,j,i)/sponge_tau*(sponge_lat-lat_now)/5;
          du(IM2,k,j,i) -= dt*w(IDN,k,j,i)*w(IVY,k,j,i)/sponge_tau*(sponge_lat-lat_now)/5;
          du(IM3,k,j,i) -= dt*w(IDN,k,j,i)*w(IVZ,k,j,i)/sponge_tau*(sponge_lat-lat_now)/5;
        } else {  // viscosity
